...
async def list_deliveries(repo: Repo, admin: AdminDependency, limit: int = 100) -> list[Delivery]:
    return await repo.list_deliveries(limit=limit)


@router.get("/{delivery_id}", response_model=Delivery)
async def get_delivery(delivery_id: str, repo: Repo, admin: AdminDependency) -> Delivery:
