...
async def list_runs(repo: Repo, admin: AdminDependency, limit: int = 50) -> list[Run]:
    return await repo.list_runs(limit=limit)


@router.get("/{run_id}", response_model=Run)
async def get_run(run_id: str, repo: Repo, admin: AdminDependency) -> Run:
...
async def trigger_run(background_tasks: BackgroundTasks, admin: AdminDependency) -> dict:
